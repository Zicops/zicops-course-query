...
RUN go mod download
COPY . .
RUN CGO_ENABLED=1 GOOS=linux go build -a -installsuffix cgo -o zicops-course-query .
...
COPY --from=builder /go/src/app/zicops-course-query /usr/bin/
EXPOSE 8091

ENTRYPOINT ["/usr/bin/zicops-course-query"]
